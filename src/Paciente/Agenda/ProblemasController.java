...
public class ProblemasController implements Initializable {
    ...
    @FXML
    private TableView<Problema> table;
    ...
    void buscar(KeyEvent event) {
        ...
        table.setItems(metodos.getProblemas(formas[comboBus.getSelectionModel().getSelectedIndex()], tbus.getText()));
        
        metodos.close();
    }

    @FXML
    void cadastrarProblema(ActionEvent event) throws IOException {
        new ScreenController().mudaBorderDireitaBaixo("/Cadastros/CadProblema.fxml");
    }
    ...
    void excluir(ActionEvent event) {
        if(table.getSelectionModel().isEmpty()){
            ...
            alert.setHeaderText("Selecione algum problema para excluir");
            ...
            alert.showAndWait().ifPresent(e->{
                if(e==sim){
                    ObservableList<Problema> lista=table.getSelectionModel().getSelectedItems();
                    metodos.start();
    ...
    void resetar(ActionEvent event) {
        ...
        table.setItems(metodos.getProblemas("''", ""));
    ...
    void visualizar(ActionEvent event) throws IOException {
        if(table.getSelectionModel().isEmpty()){
            Alert alert=new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Alerta");
            alert.setHeaderText("Selecione alguma problema para visualizar");
            alert.initOwner(ScreenController.getStage());
            alert.showAndWait();
        }
        else{
            Sessao.setId(table.getSelectionModel().getSelectedItem().getId());
            new ScreenController().mudaBorderDireitaBaixo("/Visualizadores/VisProblema.fxml");
        }
    }
